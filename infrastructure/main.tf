...
resource "aws_vpc" "aron_vpc" { 
    ...
    tags = {
        Name = "aron_vpc-${random_id.random.dec}"
    }
    lifecycle {
...
resource "aws_internet_gateway" "aron_internet_gateway" {
    ...
    tags = {
        Name = "aron_igw-${random_id.random.dec}"
    }
}
...
resource "aws_subnet" "aron_public_subnet" {
    ...
    tags = {
        Name = "aron-public-${count.index+1}"
    }
}
...
resource "aws_subnet" "aron_private_subnet" {
    ...
    tags = {
        Name = "aron-private-${count.index+1}"
    }
}
...
resource "aws_security_group_rule" "ingress_all" {
    ...
    security_group_id = aws_security_group.aron_sg.id
}
